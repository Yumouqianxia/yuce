-- Fix matches table structure to match Go code expectations
...
-- Create new matches table structure
...
-- Migrate existing data (convert field names from backup table)
INSERT INTO matches (
    team_a, 
    team_b, 
    tournament, 
...
SELECT 
    ...
    CASE 
        ...
        WHEN tournamentType = 'worlds' THEN 'WORLDS'
        ELSE 'SPRING'
    END as tournament,
    CASE 
        ...
        ELSE 'UPCOMING'
    END as status,
    STR_TO_DATE(matchTime, '%Y-%m-%d %H:%i:%s') as start_time,
    CASE 
        WHEN winner = 'optionA' THEN 'A'
        WHEN winner = 'optionB' THEN 'B'
        ELSE NULL
    END as winner,
    scoreA as score_a,
    scoreB as score_b,
...
-- Insert some test data (if no existing data)
...
-- Drop backup table (optional, keep for rollback if needed)
