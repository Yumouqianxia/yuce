...
const pageTitle = computed(() => {
  ...
  return `${year}KPL${tournamentNames[tournamentType] || ''}历史比赛`
})
...
const fetchMatches = async () => {
  ...
  try {
    ...
    console.log('历史比赛:', matches.value)
  } catch (error) {
    console.error('获取历史比赛失败:', error)
    ElMessage.error('获取历史比赛失败')
  } finally {
    loading.value = false
  }
}
